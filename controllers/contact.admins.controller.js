...
const sendMessage = async (req, res, next) => {
  ...
  try {
    if (type === "client") {
      ...
      const clientData = await pool `SELECT first_name,last_name FROM users WHERE id=${userId}`;
      ...
      const vendorData = await pool `SELECT first_name,last_name FROM vendor WHERE id=${userId}`;
